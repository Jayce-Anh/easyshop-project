...
resource "aws_cloudfront_distribution" "cf_distribution" {
  ...
  # Origin 2: ALB over HTTP (CloudFront already terminates TLS for users)
  origin {
  ...
  aliases             = ["${var.project.name}.jayce-lab.works"]
  ...
  # Ordered behaviors: backend API paths → ALB (mirrors ALB listener_rule.tf path_patterns)
...
resource "aws_route53_record" "cloudfront" {
  ...
  name    = "${var.project.name}.jayce-lab.works"
  type    = each.value
