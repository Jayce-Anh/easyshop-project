...
import { getApiUrl } from "@/lib/fetchDataFromApi";
...
const FeaturedProducts = ({ featured }: FeaturedParams) => {
  ...
  const activeShop = featured || searchParams.get("featured") || "gadgets";
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        setLoading(true);
        const res = await fetch(getApiUrl(`/products/featured?category=${activeShop}`));
